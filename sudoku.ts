...
function getValuesFromUIBoard() {
    Array.from(document.getElementsByClassName('choice')).map((cell) => {
        let newCell:Cell = cell;
        const coordinates = newCell.name.split('-');
        document.getElementById(newCell.name).classList.remove('bold');
        board[coordinates[0]][coordinates[1]] = newCell.value;
        if (newCell.value !== '') {
            document.getElementById(newCell.name).classList.add('bold');
        }
    });
    return true
}
...
function pushValuesToUIBoard() {
    for (let i = 0; i < N*N; i++) {
        for (let j = 0; j < N*N; j++) {
            const id = i + '-' + j;
            let newCell:Cell = document.getElementById(id);
            newCell.value = board[i][j]
        }
    }
    return true
}
...
function random() {
    ...
    Array.from(document.getElementsByClassName('choice')).map((cell) => {
        let newCell:Cell = cell;
        let coordinates = newCell.name.split('-');
        document.getElementById(newCell.name).classList.remove('bold');
